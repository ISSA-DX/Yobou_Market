// VariantsAccordion
// ---------------------------------------------------------------------------
// Collapsed-by-default wrapper around VariantsEditor. Lives next to the
// Stock field in the Identity card so the user sees the optional feature
// without scrolling past the rest of the form. The header shows a count
// chip; expanding the panel reveals the full editor (quick-add row,
// variant list, clear-all).
//
...
// not a monorepo. The two implementations are kept identical by copy.
...
export default function VariantsAccordion({ form, update, errors = {} }) {
  ...
  // offending rows stay hidden inside the collapsed panel. The form
  // scrolls to the panel too (handled in ProductNew.submit).
  useEffect(() => {
