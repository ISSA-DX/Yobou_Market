// CategoryPicker
// ---------------------------------------------------------------------------
// Source-of-truth dropdown for product categories. Replaces the legacy
// `<input list="category-suggestions">` (which was a hardcoded `<datalist>`
// hint, not a real picker — clicking the chevron did nothing).
//
// Behaviour:
//   - Reads the curated list from GET /api/categories (admin-curated).
//   - "Create new category…" opens a small modal that POSTs /api/categories
//     and selects the freshly-created row.
//   - Selecting an existing row calls `onChange(name)` (we keep `name` — the
//     server still writes the human label into Product.category, and the
//     shopper apps already filter by name).
//
// The picker lives in each app separately rather than in a shared package
// because the three apps are separate Vite projects, not a monorepo. The
// implementations are kept identical by copy.
...
export default function CategoryPicker({ value, onChange }) {
  ...
  async function createCategory(e) {
    ...
    try {
      ...
      // Prefer the server's structured message when available.
      ...
      <select
        ...
        onChange={(e) => {
          if (e.target.value === '__create__') {
            setCreating(true);
            // Reset the select visually so reopening the modal doesn't
            // appear to have "selected" the create option.
