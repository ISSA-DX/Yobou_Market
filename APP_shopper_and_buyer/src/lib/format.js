...
// Currencies that do not use minor units.
...
function getLocale(currency, language) {
  ...
  if (language) {
    // Compose a BCP-47 tag from the user's language and the currency's region.
...
export function currencySymbol(currency = 'USD') {
  try {
    return new Intl.NumberFormat('en-US', { style: 'currency', currency }).formatToParts(0)
