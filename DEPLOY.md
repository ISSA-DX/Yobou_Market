...
The release APK is signed by a keystore that **must never be committed
to the repo**. The signing material is loaded from GitHub Actions
secrets at build time:

- `YOBOU_KEYSTORE_BASE64` — base64-encoded `yobou-release.keystore`
- `YOBOU_KEYSTORE_PASSWORD` — store password
- `YOBOU_KEY_ALIAS` — key alias (typically `yobou`)
- `YOBOU_KEY_PASSWORD` — key password

Local release builds can read the same values from
`android/local.properties` (gitignored).

If the keystore is **rotated** (e.g. compromised or expired), every
existing install must be **uninstalled** before the new APK installs —
Android refuses to upgrade across signing-cert mismatches. Announce
this in the release notes and consider bumping `versionCode` /
